				...
				t_ray *shadow_ray, t_hit_record *shadow_ray_rec)
{
	...
	if (!is_ray_hit(world, shadow_ray, shadow_ray_rec))
		return (0);
	\
	// must also check that len(1st point to light) < len(1st point to 2nd point)
	t_vector3	A;
	v_copy(&A, &world->light.point);
	v_sub(&A, &rec->point);

	t_vector3	B;
	v_copy(&B, &shadow_ray_rec->point);
	v_sub(&B, &rec->point);

	return (v_norm2(&A) > v_norm2(&B));
